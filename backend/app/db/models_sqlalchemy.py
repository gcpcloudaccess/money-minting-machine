"""Default persistence backend: SQLAlchemy + SQLite (or any SQL DB via
DATABASE_URL). Active whenever Settings.use_firestore is False (the default -
see app/config.py). Moved verbatim from the original app/db/models.py when
Firestore was added as an alternate backend (app/db/models_firestore.py) -
nothing in this file changed, it's the exact same schema as before."""
